...
// 
const aInsertArticle = (client) => {
    console.log(client)
    return axios.post(`${API_URL}/articulos/`, client).then((response) => response.data);
}
...
const delArticleByCod = (id) => {
    console.log("DEL CLIENTE: "+`${API_URL}/articulos/${id}}`)
    return axios.delete(`${API_URL}/articulos/${id}`).then((response) => response.data);
}
...
export default {aInsertArticle, delArticleByCod};